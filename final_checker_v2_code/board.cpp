...
void Board::server_receive_MOVE(QTcpSocket * socket, NetworkData data)
{
    ...
    if (data.data2 == "")
    {
        server_send_ERROR(socket, "Sorry, your move track is null");
        return;
    }
...
void Board::server_send_ERROR(QTcpSocket * socket, QString detail)
{
    ...
    if (detail == "Sorry, your move track is null")
    {
        mysystem->server->send(socket, NetworkData(OPCODE::ERROR_OP, "NULL_TRACK", detail));
    }
}
