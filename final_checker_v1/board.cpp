...
{
    if (this->inWhichRoom->mode == 3 && !START_GAME)  //如果是网络模式，并且还没有初始化一局游戏，直接return（避免点击到还没初始化的坐标）
    {
        return;
    }

    if (this->inWhichRoom->mode == 3 && this->isServer == true)  //服务端不设置鼠标点击功能
    {
        return;
    }

    if (this->inWhichRoom->mode == 1)
    {
        // realeseTime++;
        ...
        if (this->inWhichRoom->curUser->jumpTime != 0)
        {
            // qDebug() << "curUser's jumpTime != 0--------------------------------------------------------";
            qDebug() << "curUser'jumpTime is" << this->inWhichRoom->curUser->jumpTime;
                ...
                // 如果点在区域内,注意:这里有一个非常易错的地方，在我们的坐标系内，QPoint的纵坐标越大，Coord的纵坐标越小
            {
                ...
                // 撤销所有的可行性设置
                this->inWhichRoom->cancelFeasibility();

                // 撤销用户已经选择的棋子
                this->inWhichRoom->cancelChoose();

                // 要将这位用户的跳动次数置为0
                this->inWhichRoom->curUser->jumpTime = 0;
        ...
        // 点到了我们定义的coords
        // 如果当前界面没有选中的棋子
        if (this->inWhichRoom->curChosenCoord.coord_x == -100)
        {
            ...
            if (curCoord.getColor() != null)
            {
                ...
                if (int(curCoord.getColor()) != int(this->inWhichRoom->curUser->getUserColor()))
                {
                    qDebug() << "not my piece";
                    return;
                }
                // 如果点到的是自己的棋子，将其设为选中，并展示可行性
                else if (int(curCoord.getColor()) == int(this->inWhichRoom->curUser->getUserColor()))
                {
                    qDebug() << "choose my piece--------------------------------------------------------------";

                    this->inWhichRoom->choose(curCoord);
                    this->inWhichRoom->search();  // search 依赖于curChosenUser指针，不用传参
                    update();        // 更新
                    return;
                }
            }
        ...
        else
        {
            ...
            if ((curCoord.coord_x == this->inWhichRoom->curChosenCoord.coord_x)
                    && (curCoord.coord_y == this->inWhichRoom->curChosenCoord.coord_y)
                    && this->inWhichRoom->curUser->jumpTime == 0)
            {
            ...
            if (int(curCoord.getColor()) == int(this->inWhichRoom->curUser->getUserColor()) && this->inWhichRoom->curUser->jumpTime == 0)
            {
                this->inWhichRoom->cancelFeasibility();
                this->inWhichRoom->cancelChoose();
                this->inWhichRoom->choose(curCoord);
            ...
            if (curCoord.getFeasibility() == true)
            {
                ...
                for (int i = 0; i <= 20; i++)
                {
                    ...
                    if (curCoord == this->inWhichRoom->curUser->moveTrack[i][1] || curCoord == this->inWhichRoom->curUser->moveTrack[1][0])
                    {
                        return;
                    }
                }
                ...
                move(this->inWhichRoom->curChosenCoord, curCoord);  //移动（我们允许在客户端先显示移动以后的画面，如果服务端检测到异常再进行强制遣返）

                //如果进行的是间隔跳跃
                if (abs(this->inWhichRoom->curChosenCoord.coord_x - curCoord.coord_x) >= 4
                        || abs(this->inWhichRoom->curChosenCoord.coord_y - curCoord.coord_y) >= 2)
                {
                    this->inWhichRoom->curUser->jumpTime++;  //跳跃次数+1

                    //记录轨迹
                    this->inWhichRoom->curUser->moveTrack[this->inWhichRoom->curUser->jumpTime][0] = this->inWhichRoom->curChosenCoord;
                    this->inWhichRoom->curUser->moveTrack[this->inWhichRoom->curUser->jumpTime][1] = curCoord;

                    //在用户点击next时才将jumpTime置为0（这里不置零）

                    this->inWhichRoom->cancelChoose();  //撤销已经选择的棋子
                    this->inWhichRoom->cancelFeasibility();  //撤销所有的可行性标注
                    this->inWhichRoom->choose(curCoord);      //选择跳到的位置作为当前选择的位置
                    this->inWhichRoom->search();              //搜索可行性
                    this->update();
                    return;
                }
                ...
                {
                    this->inWhichRoom->curUser->jumpTime++;  //跳跃次数++
                    this->inWhichRoom->curUser->moveTrack[this->inWhichRoom->curUser->jumpTime][0] = this->inWhichRoom->curChosenCoord; //记录轨迹
                    this->inWhichRoom->curUser->moveTrack[this->inWhichRoom->curUser->jumpTime][1] = curCoord;
                    this->inWhichRoom->cancelChoose();  //撤销已经选择的棋子
                    this->inWhichRoom->cancelFeasibility();  //撤销所有的可行性标注
                    this->inWhichRoom->next();
    ...
    if (this->inWhichRoom->mode == 3 && this->isServer == false)
    {
        // debug语句：鼠标点击次数
        // realeseTime++;
        // qDebug() << "you click " << realeseTime << " times";
        ...
        if (this->inWhichRoom->curUser != me)  // 客户端应该增加的新功能:如果curUser和me指针指向的不是users数组中的同一个玩家对象，那么直接return，无视用户的所有操作
        {
            return;
        }
        ...
        if (me->jumpTime != 0)    //如果当前用户不是第一次跳跃
        {
...

            {
                ...
                this->inWhichRoom->cancelFeasibility();   // 撤销所有的可行性设置

                this->inWhichRoom->cancelChoose();   // 撤销用户所有的选择
                ...
                client_send_MOVE();     //发送信息 //next();

                me->jumpTime = 0;  // 要将自己的跳动次数置为0

                this->update();               //这里允许现在客户端先显示移动的图像，如果出错再进行撤回
        ...
        if (this->inWhichRoom->curChosenCoord.coord_x == -100)  //如果当前界面没有被选中的棋子
        {
            qDebug() << "have not Chosen piece-----------------------------------------------------";
        ...
        else       // 如果当前界面已经有选中的棋子了
        {
            ...
            if (curCoord.getFeasibility() == true) // 如果点到的是能到达的位置,绘制新的棋子布局图像,记录到轨迹中
            {
                ...
                move(this->inWhichRoom->curChosenCoord, curCoord);  //允许自己的客户端先进行移动

                if (abs(this->inWhichRoom->curChosenCoord.coord_x - curCoord.coord_x) >= 4
                        || abs(this->inWhichRoom->curChosenCoord.coord_y - curCoord.coord_y) >= 2)  //如果进行的是间隔跳跃
                {
                    me->jumpTime++;   //当点击next时会把jumpTime置为0

                    //记录轨迹（网络模式下，应该将move记录到自己的moveTrack里面）
                    me->moveTrack[me->jumpTime][0] = this->inWhichRoom->curChosenCoord;
                ...
                else
                {
                    ...
                    this->inWhichRoom->cancelChoose();  //撤销已经选择的棋子
                    this->inWhichRoom->cancelFeasibility();  //撤销所有的可行性标志
                    this->update();              //重绘（允许在自己的客户端重绘）
...
Coord& Board::getCoord(QPoint& curPoint)
{
    for (int i = 0; i <= 24; i++)     // 枚举行
    {
        ...
        {
            // 如果这个位置是有棋槽的
            if (this->inWhichRoom->coords[i][j].getHaveSlot() == true)
            {
                // distance是用户点击点和当前枚举的棋槽点的距离
                int distance =
                    (curPoint.x() - this->inWhichRoom->coords[i][j].coord2QPoint().x()) *
                    (curPoint.x() - this->inWhichRoom->coords[i][j].coord2QPoint().x()) +
                    (curPoint.y() - this->inWhichRoom->coords[i][j].coord2QPoint().y()) *
                    (curPoint.y() - this->inWhichRoom->coords[i][j].coord2QPoint().y());

                // 距离小于棋槽半径则返回引用
                if (distance <= SLOTR * SLOTR)
                {
                    qDebug() << "you click coords:" << this->inWhichRoom->coords[i][j].coord_x << " " <<
                             this->inWhichRoom->coords[i][j].coord_y << " " << "\n";
                    return this->inWhichRoom->coords[i][j];
                }
            }
        }
    }
...
void Board::nextImag(QPainter& painter)
{
    ...
    if (this->inWhichRoom->curUser->getUserColor() == uyellow)
    {
        // qDebug() << "Enter nextImag,the User color is yellow";
        painter.setBrush(QBrush(QPixmap(YELLOW)));
    }
    else if (this->inWhichRoom->curUser->getUserColor() == ured)
    {
        // qDebug() << "Enter nextImag,the User color is red";
        painter.setBrush(QBrush(QPixmap(RED)));
    }
    else if (this->inWhichRoom->curUser->getUserColor() == upurple)
    {
        // qDebug() << "Enter nextImag,the User color is purple";
        painter.setBrush(QBrush(QPixmap(PURPLE)));
    }
    else if (this->inWhichRoom->curUser->getUserColor() == ublue)
    {
        // qDebug() << "Enter nextImag,the User color is blue";
        painter.setBrush(QBrush(QPixmap(BLUE)));
    }
    else if (this->inWhichRoom->curUser->getUserColor() == ucyan)
    {
        // qDebug() << "Enter nextImag,the User color is cyan";
        painter.setBrush(QBrush(QPixmap(CYAN)));
    }
    else if (this->inWhichRoom->curUser->getUserColor() == ugreen)
    {
        // qDebug() << "Enter nextImag,the User color is green";
        painter.setBrush(QBrush(QPixmap(GREEN)));
    }
    else if (this->inWhichRoom->curUser->getUserColor() == unull)
    {
...
void Board::reConnect()
{
    ...
    this->socket->hello("10.46.46.49", 16666);
}
...
{
    this->socket->send(NetworkData(OPCODE::LEAVE_ROOM_OP, me->userName, this->inWhichRoom->roomNumber));
}
...
void Board::server_receive_JOIN_ROOM(QTcpSocket * socket, NetworkData data)
{
    ...
    if (this->inWhichRoom->userCntInRoom == 6)  //房间已满，如果正在游戏中也显示房间已满（data1表示房间号，写多线程的时候要用到） //这里本来应该写userCnt
    {
    ...
    {
        int repeatTime = -1;
        if (this->inWhichRoom->playerList[i].userName == data.data2)  //重名
        {
            repeatTime++;
        }
        if (repeatTime >= 0)
    ...
    for (int i = 1; i <= this->inWhichRoom->userCntInRoom; i++)
    {
        if (this->inWhichRoom->playerList[i].userName != "") //只要用户名非空就代表有人(这个判断其实不必要)
        {
            server_send_JOIN_ROOM(this->inWhichRoom->playerList[i].socket, data.data2); //data2表示用户名
        }
    }
    ...
    {
        tempNameList.append(this->inWhichRoom->playerList[i].userName);  //将玩家的名字写入玩家名字列表
        tempNameList.append(" ");                     //在玩家名字的中间加入一个空格
        tempReadyList.append(QString::number(int(this->inWhichRoom->playerList[i].isReady)));  //bool->int->QString,将玩家的准备状态写入准备列表
    }
    ...
    // 执行在服务端系统中将玩家加入房间
    this->inWhichRoom->userCntInRoom++;
    this->inWhichRoom->playerList[this->inWhichRoom->userCntInRoom].userName = data.data2;   //存入名字，由于playerList采用类链表处理，故最后一个位置就是可以加入的位置
    this->inWhichRoom->playerList[this->inWhichRoom->userCntInRoom].socket = socket;
}
...
void Board::server_receive_PLATER_READY(QTcpSocket * socket, NetworkData data)
{
    ...
    if (readycnt == this->inWhichRoom->userCntInRoom)     //如果所有人都已经点击准备
    {
        ...
        this->inWhichRoom->initUsers();                   //确定了玩家人数就可以开始分配颜色
        ...
        for (int i = 1; i <= this->inWhichRoom->userCnt; i++)
        {
            server_send_START_TURN(this->inWhichRoom->playerList[i].socket);
        }
...
void Board::server_receive_LEAVE_ROOM(QTcpSocket * socket, NetworkData data)
{
    ...
    for (int i = 1; i <= 6; i++)  //重新安排房间中剩下的所有用户的座位向前对齐
    {
        ...
        {
            this->inWhichRoom->playerList[i].userName = "";    //将这名用户的姓名置空
            this->inWhichRoom->playerList[i].socket = NULL;    //将这名用户的指针置空
            this->inWhichRoom->playerList[i].isReady = false; //将准备状态设置为未准备
            ...
            //列表末端空出来一个位置，直接将它置空
            this->inWhichRoom->playerList[this->inWhichRoom->userCntInRoom].userName = "";
            this->inWhichRoom->playerList[this->inWhichRoom->userCntInRoom].socket = NULL;
            this->inWhichRoom->playerList[this->inWhichRoom->userCntInRoom].isReady = false;
        }
    }
    ...
    for (int i = 1; i <= this->inWhichRoom->userCntInRoom; i++)
    {
        server_send_LEAVE_ROOM(this->inWhichRoom->playerList[i].socket, data.data1);
    }
}
...
//        }
    ...
    for (int i = 0; i <= 24; i++)
    {
        ...
        {
            if (int(tempTargetColor) == int(this->inWhichRoom->coords[i][j].targetColor))   // 如果这个棋槽的颜色和玩家的目标色相吻合
            {
    ...
    if (overCnt == this->inWhichRoom->userCnt)   //如果所有玩家都结束了
    {
        //制作胜利者列表
        QString tempWinnerList = "";
        for (int i = 1; i <= 6; i++)
        {
            if (this->inWhichRoom->playerList[i].userName != "") // 如果当前玩家的名字不为空
            {
                tempWinnerList.append(this->inWhichRoom->playerList[i].userName);  //将这名用户的名字加到玩家列表字符串中
...
void Board::server_find_TIME_OUT()
{
    ...
    for (int i = 6; i >= 1; i--)
    {
        if (this->inWhichRoom->playerList[i].userName == "")
        {
            this->inWhichRoom->playerList[i].userName = this->inWhichRoom->curUser->userName;
            break;             //只添加一次
        }
    }
