    ...
    ui(new Ui::ask_join_room)
{
    ...
    connect(mysystem->curRoom->board, &Board::forbidButton, this, &ask_join_room::forbidButton);

    //当用户接受到已经成功进入房间的信号时，激活Ready按键
    connect(mysystem->curRoom->board, &Board::Ready_is_enabled, ui->Ready, &QPushButton::setEnabled);
    ...
    connect(this, &ask_join_room::sendReady, mysystem->curRoom->board, &Board::client_send_PLAYER_READY);

    //一旦用户点击leave就离开房间
    connect(ui->Leave, &QPushButton::clicked, mysystem->curRoom->board, &Board::client_send_LEAVE_ROOM);
    connect(ui->Leave, &QPushButton::clicked, this, &ask_join_room::init);
    connect(ui->Leave, &QPushButton::clicked, this, &ask_join_room::clickLeave);
...
void ask_join_room::init()
{
    ...
    for (int i = 1; i <= 6; i++)
    {
        mysystem->curRoom->playerList[i].userName = "";
        mysystem->curRoom->playerList[i].isReady = false;
        mysystem->curRoom->playerList[i].socket = nullptr;
    }
...
void ask_join_room::setRoomNumber()
{
    ...
    myRoomNumber = ui->roomNum->text();

    ui->roomNum_show->setText(myRoomNumber);
}
...
void ask_join_room::clickEnter()
{
    ...
    mysystem->curRoom->board->client_send_JOIN_ROOM();
}

void ask_join_room::clickLeave()
{
    this->ui->Leave->setVisible(false);
    this->ui->Ready->setEnabled(true);
    this->ui->Ready->setEnabled(true);
    this->ui->Enter->setVisible(true);
    this->ui->Enter->setEnabled(true);
    this->ui->ranking->setVisible(false);

    mysystem->curRoom->userCntInRoom = 0;  // 如果强制退出就不包含这个效果
}
...
void ask_join_room::updateTable()
{
    ...
    {
        //如果用户的名字不为空就进行显示
        if (!mysystem->curRoom->playerList[i].userName.isEmpty())
        {
            if (i == 1)
            {
                ui->AName->setText(mysystem->curRoom->playerList[i].userName);
            }

            if (i == 2)
            {
                ui->BName->setText(mysystem->curRoom->playerList[i].userName);
            }

            if (i == 3)
            {
                ui->CName->setText(mysystem->curRoom->playerList[i].userName);
            }

            if (i == 4)
            {
                ui->DName->setText(mysystem->curRoom->playerList[i].userName);
            }

            if (i == 5)
            {
                ui->EName->setText(mysystem->curRoom->playerList[i].userName);
            }

            if (i == 6)
            {
                ui->FName->setText(mysystem->curRoom->playerList[i].userName);
            }
        }
        else
        {
    ...
    {
        //如果用户已经准备好了，就为这名用户打勾
        if (mysystem->curRoom->playerList[i].isReady)
        {
            if (i == 1)
            {
        ...
        else
        {
            if (i == 1)
            {
                ui->ready1->setPixmap(QPixmap("a"));
            }

            if (i == 2)
            {
                ui->ready2->setPixmap(QPixmap("a"));
            }

            if (i == 3)
            {
                ui->ready3->setPixmap(QPixmap("a"));
            }

            if (i == 4)
            {
                ui->ready4->setPixmap(QPixmap("a"));
            }

            if (i == 5)
            {
                ui->ready5->setPixmap(QPixmap("a"));
            }

            if (i == 6)
            {
                ui->ready6->setPixmap(QPixmap("a"));
            }
        }
    }
}
...
void ask_join_room::showWINNER_LIST(QString detail)
{
    ...
    //将名字写到playerList中
    for (int i = 1; i <= mysystem->curRoom->userCnt; i++)
    {
        mysystem->curRoom->playerList[i].userName = detail.section(" ", i - 1, i - 1);
    }
    ...
    ui->AName->setText(mysystem->curRoom->playerList[1].userName);
    ui->BName->setText(mysystem->curRoom->playerList[2].userName);
    ui->CName->setText(mysystem->curRoom->playerList[3].userName);
    ui->DName->setText(mysystem->curRoom->playerList[4].userName);
    ui->EName->setText(mysystem->curRoom->playerList[5].userName);
    ui->FName->setText(mysystem->curRoom->playerList[6].userName);
