    ...
    ui(new Ui::ShowInfo)
{
    ...
    //设置窗口居中
    QPoint globalPos = mysystem->curRoom->board->mapToGlobal(QPoint(0, 0)); //父窗口绝对坐标
    int x = globalPos.x() + (mysystem->curRoom->board->width() - this->width()) / 2;//x坐标
    int y = globalPos.y() + (mysystem->curRoom->board->height() - this->height()) / 2;//y坐标
    this->move(x, y);//窗口移动
    ...
    connect(mysystem->curRoom->board, &Board::client_receive_ERROR, this, &ShowInfo::receiveERROR);
    ...
    connect(ui->reConnect, &QPushButton::clicked, mysystem->curRoom->board, &Board::reConnect);
...
void ShowInfo::showINVALID_MOVE(QString detail)
{
    ...
    mysystem->curRoom->board->movePieceBack();
}
